...
def calcular_f1_score(img_segmentada, img_groundtruth):
...
def calcular_boundary_f1_score(img_segmentada, img_groundtruth):
...
#Método para mostrar la media de los resultados de f1 score y boundary f1 score
def mostrar_resultados_metricas(f1_valores,boundary_f1_valores):
...
#Método para mostrar gráfica de puntos
def mostrar_grafica_puntos(f1_valores,boundary_f1_valores):
...
if __name__ == "__main__":
    ...
    for image_name in os.listdir(carpeta_segmentaciones_modelo):
        ...
        # F1 Score
        try:
            f1 = calcular_f1_score(img_segmentada,img_groundtruth)
            if f1 > max_f1_valor:
                max_f1_valor = f1
        ...
        #Boundary F1 Score
        try:
            boundary_f1 = calcular_boundary_f1_score(img_segmentada,img_groundtruth)
            boundary_f1_valores.append(boundary_f1)
            print(image_name," :: Boundary F1-score: ",boundary_f1)
        except:
            print(image_name)
            continue


    mostrar_resultados_metricas(f1_valores,boundary_f1_valores)
    mostrar_grafica_puntos(f1_valores,boundary_f1_valores)